...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 18 20:38:17 2020
...
''' create synthetic correlated dataset using sklearn make_blobs:
    provides us with ways to test the algorithm'.
    in the following code:
        N the data set size
        data countains the generated data
        G the correlation matrix
        gdic is a dictionary with equal content to G'''
    
N = 10000
data,_ = make_blobs(n_samples=N,n_features=500,shuffle=False,centers=5,random_state=0)
...
ns_ = np.ones(N).astype(int).tolist()
...
while len(other_keys) >1:
    ...
    ''' a random initialization:
        ...
        then loop through the other objects using 'others' and costs to store
    ...
    others = [i for i in other_keys]
    others.remove(node)
    costs = np.zeros(len(others))
    indices = [(node,key) for key in others]
    ...
    if costs[next_merge]<=0:
        ...
        ''' One a cluster is formed, the correlation matrix gdic needs updating
        and the objects (label_a and label_b) include in new_label must be removed
        from other_keys, and the cluster size must be updated with ns_'''
